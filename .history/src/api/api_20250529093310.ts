...
const checkOrderByEmail = async (email: string): Promise<Order | null> => {
	try {
		const response = await requestResponse<Order>(`/api/order-safe/${email}`, "GET");
		return response.response;
	} catch (error: any) {
		// If 404 or "not found", return null silently
		if (
		...
		console.error("Unexpected error in checkOrderByEmail:", error);
		return null;
	}
};
