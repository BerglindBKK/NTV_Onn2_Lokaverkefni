...
const Home = () => {
	...
	const fetchOrderByEmail = async (email: string) => {
		...
		try {
			order = await api.getOrderByEmail(email);
		} catch (err: any) {
			const isNotFound =
				err?.response?.status === 404 ||
				err?.message?.includes("Order not found") ||
				err?.toString()?.includes("Order not found");

			if (!isNotFound) {
				// Only show unexpected issues
				console.error("Unexpected API error:", err);
			}

			// For "not found" we just return silently
			setOrder(null);
			setIsExistingOrder(false);
			return;
		}

		// Only run this if no error
