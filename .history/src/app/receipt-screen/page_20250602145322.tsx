...
const ReceiptScreen = () => {
    ...
    const handleDelete = async (email: string) => {
        ...
        setLoading(true);
        try {
            //encodeURIComponent(email) converts special characters in the email (like @ and .) into a format that is safe to include in a URL, preventing errors when sending it as part of the URL path.
            const deleted = await api.deleteOrderByEmail(encodeURIComponent(email));
            console.log("Delete response:", deleted);
            if (deleted && deleted.success && deleted.deletedorder) {
                alert("Order deleted");
                router.push("/");
            } else {
            ...
            if (err.message) {
                alert(`Something went wrong while deleting the order: ${err.message}`);
            } else {
