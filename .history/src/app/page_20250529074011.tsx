...
const Home = () => {
	...
	const fetchOrderByEmail = async (email: string) => {
		try {
			...
			const isNotFound =
				err?.response?.status === 404 || // Axios style
				err?.message?.includes("Order not found"); // Custom error message fallback

			if (isNotFound) {
				localStorage.removeItem("userEmail");
				setOrder(null);
				setIsExistingOrder(false);
				return; // do nothing else, silently handle
			}

			// Other unexpected errors should still be logged or handled
