//TODO!! Setja inn UI sem sýnir hvort það sé verið að breyta pöntun eða gera nýja. 
...
const SelectDish = () => {
    ...
    //if there is an existing order and dish, then set all the dish info equal to the info in the order
    //if there is no existing order, then reun fetchdish to generate a random dish
    ...
    useEffect(() => {
        ...
        if (storedEmail) {
            fetchOrderByEmail(storedEmail).then(() => {
                if (isNew) {
                    fetchDish(); // force-fetch new dish
