...
const Home = () => {
	...
	const fetchOrderByEmail = async (email: string) => {
		try {
			...
			// ✅ Detect a "not found" error without logging it
			const isNotFound =
				err?.response?.status === 404 || // Axios-style 404
				err?.message?.includes("Order not found") || // Custom thrown error
				err?.toString()?.includes("Order not found"); // Catch-all string check
			...
			// ❗ Only log other errors, not "not found"
			console.error("Unexpected API error:", err);
		}
	};

	//user Email deleted from storage on mount
	useEffect(() => {
		console.log(`Previous local storage userEmail: ${userEmail}`);
		localStorage.removeItem("userEmail");
		console.log(`Local storage userEmail after removeItem:: ${userEmail}`);
	}, [])
				...
				<div className="parent-homescreen">
					...
					<div className="div4">
						...
						<div className="div4-child">
							...
							/>
							{isExistingOrder === false && userEmail && (
								<p>No existing order with this email</p>
							)}
						...
						<div className="div4-child-error" >
							{isExistingOrder ? (
								`${userEmail} exists, click the button to update order`
							) : (`no existing order found for ${userEmail}`
							)};
