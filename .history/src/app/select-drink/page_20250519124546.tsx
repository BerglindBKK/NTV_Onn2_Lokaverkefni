...
import SelectDish from "../select-dish/page";
...
//because we are displaying the selected dish on this page too:
type Dish = {
    idMeal: string;
    strMeal: string;
    strMealThumb: string;
...
const SelectDrink = () => {
    ...
    const { dishId } = useParams();        // Id from Next
    ...
    const getDrinkData = async () => {
        ...
        //also getting dish-info
        useEffect(() => {
            if (!dishId) return;
            fetch(`https://themealdb.com/api/json/v1/1/lookup.php?i=${dishId}`)
                .then(res => res.json())
                .then((json: { meals: Dish[] }) => setDish(json.meals[0]));
        }, [dishId]);
    };
