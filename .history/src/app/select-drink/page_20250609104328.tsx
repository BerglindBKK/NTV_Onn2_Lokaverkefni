...
const SelectDrink = () => {
    ...
    const [drinkAmounts, setDrinkAmounts] = useState<{ [id: string]: string }>({}); //how manay drinks
    ...
    //fetching all drinks with initial letter v and storing it in a json file 
    ...
    const getSearchData = async () => {
        try {
            ...
            const urlIngredient = `https://www.thecocktaildb.com/api/json/v1/1/filter.php?i=${search}`;
    ...
    //if no order exists - don't go any firther
    //if wxisting order and if there are drinks in the order then get the selected drinks from drinksId in the order
    //then make an empty amount object, loop through drinks from the existing order and finds the count of each drink, turns into string (because count is a number)
    //this runs every time order changes
    useEffect(() => {
        if (!order) return;

        if (order.drinks && order.drinks.length > 0) {
            const drinkIds = order.drinks.map(drink => drink.idDrink);
            setSelectedDrinkId(drinkIds);

            const amounts: { [id: string]: string } = {};
            order.drinks.forEach(drink => {
                amounts[drink.idDrink] = drink.count?.toString() || "1";
            });
            setDrinkAmounts(amounts);

            // Ensure all order drinks are in the drink list
            const missingDrinks = order.drinks.filter(
                od => !drink.some(d => d.idDrink === od.idDrink)
            );
            if (missingDrinks.length > 0) {
                setDrink(prev => [...prev, ...missingDrinks]);
            }
        }
    }, [order]);
    ...
    //combining adrink and found to display selected drinks correctly, removes possible duplicates byidDrink
    ...
    const seen = new Set();
    const uniqueDrinks = combinedDrinks.filter(drink => {
        if (seen.has(drink.idDrink)) return false;
        seen.add(drink.idDrink);
        return true;
    });


    const selectedDrinks = selectedDrinkId.map(id => {
        return uniqueDrinks.find(d => d.idDrink === id)
            || order?.drinks.find(d => d.idDrink === id)
            || null;
    ...
    const handleSelectDrink = async () => {
        ...
        const updatedOrder: Order = {
            ...
            drinks: selectedDrinks.map((drink) => ({
                ...
                .filter(drink => drink.count > 0), //if zero, dont save
        };
    ...
    const allDrinksToShow = (() => {
        const base = search.length > 0 ? found : drink;
        const baseIds = new Set(base.map(d => d.idDrink));
        const missingSelected = selectedDrinks.filter(d => !baseIds.has(d.idDrink));
        const all = [...base, ...missingSelected];
        const seen = new Set();
        const deduplicated = all.filter(drink => {
            if (seen.has(drink.idDrink)) return false;
            seen.add(drink.idDrink);
            return true;
        });
        ...
        <div className="page-container">
            <main className="grid-wrapper">
                <div className="parent-orderdish">
                    <div className="div5-drinks">
                        {/* search bar */}
                        {/* sýnir default drykki ef ekkert matchar við leitarskilyrði - breyta þannig að ekkert sést? */}
                                ...
                                {allDrinksToShow.map((im, idx) => (
                                    ...
                                    >
                                        ...
                                        {hoveredDrinkId === im.idDrink && (
                                            <div className="grid-item-fact p-drink">
                                                <p className="p-drink"><strong>{im.strDrink}</strong></p>
                                                <p className="p-drink">{im.strAlcoholic}</p><br></br>

                                                <p className="p-drink"><strong>Category:</strong></p>
                                                <p className="p-drink">{im.strCategory}</p>

                                                <p className="p-drink"><strong>Served in a:</strong></p>
                                                <p className="p-drink">{im.strGlass}</p>

                                                <p className="p-drink"><strong>Main ingredients:</strong></p>
                                                {/* displays main ingredients, can be upto 15 */}
                                                {Array.from({ length: 15 }, (_, i) => {
                                                    const key = `strIngredient${i + 1}` as keyof Drink;
                                                    const value = im[key];
                                                    return value ? <p className="p-drink" key={key}>Ingredient {i + 1}: {value}</p> : null;
                                                }
                                                )}
                                            </div>
                                        )}
                    ...
                    <div className="div2">
                        <div className="left receipt">
                            <div className="div2-mobile-hidden">
                            ...
                            {selectedDrinks.map((drink) => (
                                <div className="drink-summary"
                                    ...
                                    <div className="medium-font">
                                        <button
                                            className="p minus-one-sm"
                                            onClick={() => {
                                                setDrinkAmounts(prev => {
                                                    const current = parseInt(prev[drink.idDrink] || "0", 10);
                                                    const newAmount = Math.max(current - 1, 0);
                                                    return { ...prev, [drink.idDrink]: newAmount.toString() };
                                                });
                                            }}>-</button>
                                        {drinkAmounts[drink.idDrink] || 1}
                                        <button
                                            className="p plus-one-sm"
                                            onClick={() => {
                                                setDrinkAmounts(prev => {
                                                    const current = parseInt(prev[drink.idDrink] || "0", 10);
                                    ...
                                    <div className="medium-font">{drink.strDrink} ({drink.strAlcoholic}) x $20</div>
                                </div>
                            ))}
                        </div >
