...
const SelectDrink = () => {
    ...
    const [selectedDrinkId, setSelectedDrinkId] = useState<Drink[]>([]);
    ...
    function useIsSelected(id: string, selectedDrinkId: string[]) {
        return useMemo(() => selectedDrinkId.includes(id), [id, selectedDrinkId]);
    }
    return (
                    ...
                    <div className="div5">
                        {drink.length === 0 ? (
                            ...
                            <div className="grid-container">
                                {drink.map((im) => {
                                    // use the hook exactly as named
                                    const isSelected = useIsSelected(im.idDrink, selectedDrinkId);

                                    return (
                                        <div
                                            className={`grid-item ${isSelected ? "grid-item--selected" : ""}`}
                                            key={im.idDrink}
                                            onMouseEnter={() => setHoveredDrinkId(im.idDrink)}
                                            onMouseLeave={() => setHoveredDrinkId(null)}
                                            onClick={() =>
                                                setSelectedDrinkId((ds) => [...ds, im.idDrink])
                                            }
                                        >
                                            <img src={im.strDrinkThumb} alt={im.strDrink} />
                                            {hoveredDrinkId === im.idDrink && (
                                                <div className="grid-item-fact">{im.idDrink}</div>
                                            )}
                                        </div>
                                    );
                                })}
