...
const SelectDrink = () => {
    ...
    //checking if there is an existing order for this email
    let isEditMode;
    if (order && order.dish) {
        isEditMode = true;
    } else {
    ...
    const getSearchData = async () => {
        try {
            ...
            // If the esponse is not OK (status code 200), throw an error
    ...
    const fetchDrink = async () => {
        ...
        console.log("Fetched default drinks:", drink);
        setDrink(drink);
    };
    ...
    //if there is an existing order and dish and drinks, then set all the drinkinfo equal to the info in the order
    //if there is no existing order, then clean slate
    //only render this when order updates
    useEffect(() => {
        const storedEmail = localStorage.getItem("userEmail");
        const isNew = localStorage.getItem("isNewOrder") === "true";

        if (storedEmail) {
            fetchOrderByEmail(storedEmail).then((fetchedOrder) => {
                const hasNoDrinks = !fetchedOrder?.drinks || fetchedOrder.drinks.length === 0;
                if (isNew || hasNoDrinks) {
                    fetchDrink();
    ...
    useEffect(() => {
        ...
        if (order.dish) {
            // Edit mode
            setDrink(order.drinks);
            setDrinkAmounts(
                order.drinks.reduce((acc, drink) => {
                    if (drink.idDrink) {
                        acc[drink.idDrink] = String(drink.count ?? "0");
                    }
                    return acc;
                }, {} as { [id: string]: string })
            );
        } else {
            fetchDrink(); // keep this
        }
    }, [order]);
    ...
    console.log("Combined drinks:", combinedDrinks);
            ...
            <main className="grid-wrapper">
                <div className="parent-orderdish">
                    <div className="div5">
                        ...
                        {/* {(search.length === 0 && drink.length === 0 && found.length === 0) ? (
                            <div className="p-20">
                                <p>Loading...</p>
                            </div>
                        ) : ( */}
                    ...
                    <div className="div2">
                        <div className="left receipt">
                            ...
                            {(selectedDrinks.length > 0 ? selectedDrinks : order.drinks).map((drink) => (
