...
const Home = () => {
	...
	const fetchOrderByEmail = async (email: string) => {
		const order = await api.checkOrderByEmail(email); // Use the new safe version
		if (order) {
			order.count = order.count ?? 1; // Ensure count is defined
			setOrder(order);
	...
	useEffect(() => {
		...
		const timeout = setTimeout(() => {
			fetchOrderByEmail(userEmail); // Use your local wrapper, not api directly
		}, 600); // delay in ms
	...
	const handleCreateOrUpdateOrder = async () => {
		if (isExistingOrder) {
			// maybe update order logic or redirect
			router.push("/select-dish");
		} else {
			// create order
