...
const Home = () => {
	...
	const doesOrderExist = async (email: string): Promise<boolean> => {
		try {
			...
			return !!order; // true if found
		} catch (err: any) {
			// Instead of throwing or error handling, just return false on "not found"
			if (err.message.includes("Order not found")) {
				return false;
			}
			// Optional: If some other error occurs, you can throw or handle differently
			throw err;
		}
	};
	...
	const handleCreateOrder = async () => {
		...
		try {
			...
			if (exists) {
				...
				router.push("/update-order"); // Or wherever you want
				return;
			}
